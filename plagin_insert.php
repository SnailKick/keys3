                ...
                foreach ($media['message'] as $item) {
                    ...
                    switch ($name) {
                        case 'Количество регистрируемых телепараметров':
                            $volume_parameters = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Количество рабочих станций':
                            $users = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Платформа':
                            $platform = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Дата создания лицензии':
                            $date_creation = date('Y-m-d', strtotime($value)); // Преобразуем дату в формат yyyy-mm-dd
                            break;
                        case 'Дата разрешенного обновления до':
                            $date_sub_upgrade = date('Y-m-d', strtotime($value)); // Преобразуем дату в формат yyyy-mm-dd
                            break;
                        case 'Максимальная версия ПО':
                            $max_soft_version = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Клиент':
                            $client = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Клиент10':
                            $client10 = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Веб-клиент10':
                            $web_client10 = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Поддержка протокола МЭК 60870-5-103':
                            $protocol_support = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Поддержка OPC DA2.0 клиент/сервер':
                            $opc_support = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Поддержка Веб-клиента':
                            $web_client_support = mysql_real_escape_string($value, $conn);
                            break;
							                        case 'Протокол обмена DNP3':
                            $exchange_protocol_dnp3 = mysql_real_escape_string($value, $conn);
                            break;
                        case 'Уровень безопасности':
                            $security_level = mysql_real_escape_string($value, $conn);
                            break;
                    }
                }
